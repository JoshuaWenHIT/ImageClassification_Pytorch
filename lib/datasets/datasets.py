...
from lib import create_transform
...
def create_dataset(config: yacs.config.CfgNode,
    ...
    if config.dataset.name in [
        ...
        val_dataset = torchvision.datasets.ImageFolder(dataset_dir / 'val',
                                                       transform=val_transform)
        return train_dataset, val_dataset
    elif config.dataset.name in CUSTOM_DATASETS:
        train_transform = create_transform(config, is_train=True)
        val_transform = create_transform(config, is_train=False)
        train_dataset = torchvision.datasets.ImageFolder(
            os.path.join(DATASETS_ROOT, config.dataset.name, 'train'), transform=train_transform)
        val_dataset = torchvision.datasets.ImageFolder(
            os.path.join(DATASETS_ROOT, config.dataset.name, 'val'), transform=val_transform)
        return train_dataset, val_dataset
    else:
        raise ValueError()
